#!/usr/bin/env python3
"""
phone_agent.py – Scraptraffic voice bot for OpenAI Realtime API (v2)
────────────────────────────────────────────────────────────────
...
• Optional WebRTC-VAD if installed; pure-Python energy VAD otherwise
• Starts recording after ≥200 ms of consecutive speech
• Sends utterance when ≥600 ms of continuous speech separated by ≥2000 ms of silence
• Dynamic recalibration of ambient noise every minute when idle
• Optional “push-to-talk” mode (hold spacebar to record)
...
# Now require ~200 ms of speech to start
START_SPEECH_FRAMES    = 200 // FRAME_MS  
# And at least ~600 ms total speech to send
MIN_UTTERANCE_FRAMES   = 600 // FRAME_MS  
# End on ~2000 ms of silence
...
async def main() -> None:
    ...
    parser.add_argument("--push-to-talk", action="store_true",
                        help="Require holding spacebar to record")
    ...
    if HAVE_WEBRTC:
        ...
        # store in mutable for dynamic updates
        vad_config = {"threshold": energy_threshold}
        print(f"✅ Initial threshold: {energy_threshold:.6f}")

        # background thread to re-calibrate every minute when idle
        def ambient_recalibrator():
            while True:
                time.sleep(60)
                if not speaking.get() and not awaiting_response.get():
                    ...
                    print(f"\n🔄 Re-calibrated threshold: {vad_config['threshold']:.6f}")

        threading.Thread(target=ambient_recalibrator, daemon=True).start()
    ...
    if args.push_to_talk:
        ...
        Listener(on_press=_on_press, on_release=_on_release).start()
        print("🎤 Push-to-talk enabled: hold [SPACE] to talk")

    # ── Speech capture & turn management ───────────────────────
    in_speech      = False
    voice_frames   = 0
    silence_frames = 0
    buffer         = bytearray()
    ...
    def callback(indata, *_):
        ...
        # never record while assistant is speaking or being generated
        if speaking.get() or awaiting_response.get():
            return

        # if push-to-talk, do nothing unless SPACE is held
        ...
        if is_voice:
            ...
            if not in_speech:
                ...
                in_speech = False
                print("\n[Speech ended]")
                utter_len = len(buffer) // 2 // FRAME_SAMPLES
                if utter_len >= MIN_UTTERANCE_FRAMES:
